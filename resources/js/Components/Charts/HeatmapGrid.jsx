...
export default function HeatmapGrid({
    ...
    const rows = data;
    const max = Math.max(0, ...rows.flatMap((r) => r.hours).filter((v) => v));

    if (rows.length === 0 || max === 0) return <EmptyChart message={emptyMessage} />;

    const cols = 24 / hourStep;
    const hours = Array.from({ length: cols }, (_, i) => i * hourStep);
        ...
        <div className={className}>
            ...
            <div className="flex items-center justify-between mt-2 text-[10px] text-gray-400">
                ...
                <span className="flex items-center gap-1.5">
                    <span className="w-3 h-3 rounded-sm" style={{ background: mix(AMBER, ROSE, 0) }} />
                    <span className="w-3 h-3 rounded-sm" style={{ background: mix(AMBER, ROSE, 0.5) }} />
                    <span className="w-3 h-3 rounded-sm" style={{ background: mix(AMBER, ROSE, 1) }} />
                    Máximo
                </span>
            </div>
        </div>
